...
export function Deposit({ fetchInfo }) {
  ...
  const handleSubmit = async (e) => {
    try {
      e.preventDefault()
      await fetch(`http://localhost:3000/deposit_money_balance/${oldUserInfo.id}`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <>
      <NewButton text='Depositar' onClick={onOpen}>
      ...
      >
        <ModalOverlay />
        <ModalContent>
          <ModalHeader fontSize={'medium'}>Depositar recursos.</ModalHeader>
          <ModalCloseButton />
